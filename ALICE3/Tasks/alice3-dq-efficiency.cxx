...
struct AnalysisEventSelection {
  ...
  void runEventSelection(TEvents const& events, TEventsMC const& mcEvents)
  {
    ...
    for (auto& event : events) {
      ...
      VarManager::FillEventAlice3<TEventFillMap>(event);
...
struct AnalysisTrackSelection {
  ...
  void runTrackSelection(ReducedA3TracksAssoc const& assocs, TEvents const& /*events*/, TTracks const& tracks, TEventsMC const& /*eventsMC*/, TTracksMC const& tracksMC)
  {
    ...
    for (auto& assoc : assocs) {
      auto event = assoc.template reducedA3event_as<TEvents>();
      ...
      auto track = assoc.template reducedA3track_as<TTracks>();
      VarManager::FillTrackAlice3<TTrackFillMap>(track);
  ...
  void processSkimmed(ReducedA3TracksAssoc const& assocs, MyEventsSelected const& events, MyBarrelTracks const& tracks, ReducedA3MCEvents const& eventsMC, ReducedA3MCTracks const& tracksMC)
  {
...
struct AnalysisPrefilterSelection {
  ...
  void runPrefilter(TEvent const& event, soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts> const& assocs, TTracks const& /*tracks*/)
  {
    ...
    for (auto& [assoc1, assoc2] : o2::soa::combinations(assocs, assocs)) {
      auto track1 = assoc1.template reducedA3track_as<TTracks>();
      auto track2 = assoc2.template reducedA3track_as<TTracks>();
      ...
      VarManager::FillPairAlice3<VarManager::kDecayToEE, TTrackFillMap>(track1, track2);
      if (fPropTrack) {
  ...
  void processBarrelSkimmed(MyEvents const& events, soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts> const& assocs, MyBarrelTracks const& tracks)
  {
...
struct AnalysisSameEventPairing {
  ...
  PresliceUnsorted<soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts, aod::Prefilter>> trackAssocsPerCollision = aod::reducedA3track_association::reducedA3eventId;
  ...
  void runSameEventPairing(TEvents const& events, PresliceUnsorted<TTrackAssocs>& preslice, TTrackAssocs const& assocs, TTracks const& /*tracks*/, ReducedA3MCEvents const& /*mcEvents*/, ReducedA3MCTracks const& /*mcTracks*/)
  {
    if (events.size() == 0) {
    ...
    for (auto& event : events) {
      ...
      for (auto& [a1, a2] : o2::soa::combinations(groupedAssocs, groupedAssocs)) {
        ...
        if constexpr (TPairType == VarManager::kDecayToEE) {
          ...
          VarManager::FillPairAlice3<TPairType, TTrackFillMap>(t1, t2);
          if (fPropTrack) {
          ...
          if (!fConfigMC.skimSignalOnly || (fConfigMC.skimSignalOnly && mcDecision > 0)) {
            ...
            if constexpr (trackHasCov && TTwoProngFitter) {
              ...
              if constexpr ((TTrackFillMap & VarManager::ObjTypes::ReducedTrackCollInfo) > 0) {
                if (fConfigOptions.flatTables.value && t1.has_reducedA3MCTrack() && t2.has_reducedA3MCTrack()) {
                  
                  // Implement new Minitree with updated model
  ...
  void processBarrelOnlySkimmed(MyEventsVtxCovSelected const& events,
                                soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts, aod::Prefilter> const& barrelAssocs,
  ...
  {
    runSameEventPairing<true, VarManager::kDecayToEE, gkEventFillMapWithCov, gkTrackFillMap>(events, trackAssocsPerCollision, barrelAssocs, barrelTracks, mcEvents, mcTracks);
...
struct AnalysisAsymmetricPairing {
  ...
  PresliceUnsorted<soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts>> trackAssocsPerCollision = aod::reducedA3track_association::reducedA3eventId;
  //PresliceUnsorted<aod::ReducedA3TracksAssoc> trackAssocsPerCollision = aod::reducedA3track_association::reducedA3eventId;

  // Partitions for triplets and asymmetric pairs
  Partition<soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts>> legACandidateAssocs = (o2::aod::dqanalysisflags::isBarrelSelected & fConfigLegAFilterMask) > static_cast<uint32_t>(0);
  Partition<soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts>> legBCandidateAssocs = (o2::aod::dqanalysisflags::isBarrelSelected & fConfigLegBFilterMask) > static_cast<uint32_t>(0);
  Partition<soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts>> legCCandidateAssocs = (o2::aod::dqanalysisflags::isBarrelSelected & fConfigLegCFilterMask) > static_cast<uint32_t>(0);
  ...
  template <bool TTwoProngFitter, int TPairType, uint32_t TEventFillMap, uint32_t TTrackFillMap, typename TEvents, typename TTrackAssocs, typename TTracks>
  void runAsymmetricPairing(TEvents const& events, PresliceUnsorted<TTrackAssocs>& preslice, TTrackAssocs const& /*assocs*/, TTracks const& /*tracks*/, ReducedA3MCEvents const& /*mcEvents*/, ReducedA3MCTracks const& /*mcTracks*/)
  {
    fPairCount.clear();
    ...
    for (auto& event : events) {
      ...
      // Reset the fValues array
      VarManager::ResetValues(0, VarManager::kNVars);
      VarManager::FillEventAlice3<TEventFillMap>(event, VarManager::fgValues);
      ...
      for (auto& [a1, a2] : combinations(soa::CombinationsFullIndexPolicy(groupedLegAAssocs, groupedLegBAssocs))) {
        ...
        for (auto sig = fRecMCSignals.begin(); sig != fRecMCSignals.end(); sig++, isig++) {
          if (t1.has_reducedA3MCTrack() && t2.has_reducedA3MCTrack()) {
            VarManager::FillPairMC<TPairType>(t1.reducedA3MCTrack(), t2.reducedA3MCTrack());
            if ((*sig)->CheckSignal(true, t1.reducedA3MCTrack(), t2.reducedA3MCTrack())) {
              mcDecision |= static_cast<uint32_t>(1) << isig;
            }
          }
  ...
  void processKaonPionSkimmed(MyEventsVtxCovSelected const& events,
                              soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts> const& barrelAssocs,
  ...
  void processKaonPionSkimmedMultExtra(MyEventsVtxCovSelected const& events,
                                       soa::Join<aod::ReducedA3TracksAssoc, aod::BarrelTrackCuts> const& barrelAssocs,
                                       MyBarrelTracksWithCovWithAmbiguities const& barrelTracks,
                                       ReducedA3MCEvents const& mcEvents, ReducedA3MCTracks const& mcTracks)
  {
