...
//          Alexander Tiekoetter (atiekoet@cern.ch)
//
...
struct Alice3DQTableMaker {
  ...
  void skimCollisions(TEvents const& collisions)
  {
    ...
    for (const auto& collision : collisions) {
      ...
      VarManager::FillEventAlice3<TEventFillMap>(collision); // extract event information and place it in the fValues array
  ...
  template <uint32_t TEventFillMap, uint32_t TTrackFillMap, uint32_t TEventMcFillMap, typename TEvents, typename TTracks, typename TTrackAssoc, typename MyEvents>
  void fullSkimming(TEvents const& collisions,
                    TTracks const& tracksBarrel, TTrackAssoc const& trackAssocs, 
                    MyEvents const& mcCollisions, aod::McParticles const& mcParticles)
  {
    ...
    skimCollisions<TEventFillMap>(collisions);
  ...
  {
    fullSkimming<gkEventFillMap, gkTrackFillMapWithCov, gkEventMcFillMap>(collisions, tracksBarrel, trackAssocs, mcCollisions, mcParticles);
  }
